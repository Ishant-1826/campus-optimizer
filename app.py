...
# 2. PRISM DARK CSS (Refined for Hackathon)
st.markdown("""
    <style>
    ...
    .stButton > button:hover {
        ...
        box-shadow: 0 0 20px rgba(188, 140, 255, 0.4) !important;
    }
    ...
    .venue-available { border-left: 5px solid #238636; }
    .activity-tag { background: #161b22; color: #bc8cff; padding: 2px 8px; border-radius: 5px; font-size: 0.85em; }
    </style>
...
# --- PAGE 2: HUB (AI MATCHING + RESOURCE ENGINE) ---
    ...
    # --- NEW: CAMPUS RESOURCE ADVISOR ---
    st.divider()
    st.subheader("📍 Live Campus Resource Availability")
    
    # In a real hackathon demo, this data would come from your 'timetable' sheet tab
    campus_data = [
        {"venue": "Computer Centre (Lab 2)", "status": "Empty", "suggested": "Project Work / ML Lab"},
        {"venue": "Lecture Hall 102", "status": "No Class", "suggested": "Study Group (Math)"},
        {"venue": "Main Library (Zone B)", "status": "Low Occupancy", "suggested": "Quiet DSA Practice"}
    ]
    ...
    for i, res in enumerate(campus_data):
        with res_cols[i]:
            st.markdown(f"""
            <div class="prism-card venue-available">
                <p style="color:#238636; font-size:0.8em; margin:0; font-weight:700;">● LIVE: {res['status']}</p>
                <h3 style="margin:5px 0;">{res['venue']}</h3>
                <p>Activity: <span class="activity-tag">{res['suggested']}</span></p>
            </div>
            """, unsafe_allow_html=True)
            ...
            for i, idx_val in enumerate(indices[0]):
                ...
                with grid[i % 2]:
                    ...
                    <div class="prism-card">
                        ...
                        <p style="margin:5px 0;">Similarity: <b>{sim}%</b></p>
                        <p style="font-size:0.9em;">Focus: {p['interests']}</p>
                    </div>
                    """, unsafe_allow_html=True)
...
# --- PAGE 3: SUCCESS ---
    ...
    st.markdown(f"""
        <div style="text-align: center; background: rgba(0, 112, 243, 0.1); border: 1px solid #0070f3; padding: 50px; border-radius: 30px;">
            ...
            <p>Handshake verified. Suggested Venue: <b>Computer Centre</b></p>
        </div>
    """, unsafe_allow_html=True)
